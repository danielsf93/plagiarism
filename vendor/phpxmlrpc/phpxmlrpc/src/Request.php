...
class Request
{
    ...
    public function parseResponse($data = '', $headersProcessed = false, $returnType = 'xmlrpcvals')
    {
       ...
       // solução para 'PHP Warning:  is_countable() expects exactly 1 parameter, 2 given'
        if (!xml_parse($parser, $data) && count($data) > 0) {
       // original: if (!xml_parse($parser, $data, count($data))) {
